# 13 — Trained AI + majority vote + selection of hard questions (30%) + large AI marker, none hint

**In standard terms:** Stacked on self-consistency@3; disagreement gate at 30% escalation; GLM-4.7-Flash verifier; L0 feedback

The trained AI answers every question three times. Where at least two attempts agree, that answer is kept straight away. About 30% of questions are sent to large AI marker — the ones where the three attempts disagreed most. If the marker says the working is wrong, it replies with only yes or no, with no explanation, and the small AI tries again. The marker is never shown the correct answer.
...
| | |
|---|---|
| Right answers | **798 of 1,319 — 60.50%** |
| Compared with the trained AI answering once | +4.62 percentage points |
| Answers turned from wrong to right | 100 |
| Answers turned from right to wrong | 39 |
| AI attempts per question | 3.46 |
| Words the small AI writes per question (tokens) | 476.8 |
| Words the small AI reads per question (tokens) | 399.1 |
| Questions sent to the marker | 396 (30.0%) |
| Times the marker was asked | 712 |
| Tokens exchanged with the marker per question | 300.9  (in 268.9 / out 32.0) |
...
| **Time per question (estimated)** | 55.1 s |
| **Time for all 1,319 questions (estimated)** | 20 h 12 min |

**Where these numbers come from:** `outputs/predictions/10_pipeline_glm30b_hint_none.jsonl`. Every count, including the ones inside the
...
flowchart TB
    ...
    CHECK -->|"if wrong: it just says wrong<br/>(never the answer)"| SOLVER
...
flowchart TD
    ...
    M -->|"says wrong<br/><b>316</b>"| R1["<b>The small AI tries again</b><br/>told only that it was wrong"]
    R1 -->|"marker now says right<br/><b>22</b>"| KEEP2
    R1 -->|"still wrong<br/><b>294</b>"| R2["One last try,<br/>kept without checking"]
    K --> RES
    KEEP2 --> RES
    R2 --> RES
    RES["<b>798 right</b> (60.5%)<br/>521 wrong"]
